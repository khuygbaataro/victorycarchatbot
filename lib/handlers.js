...
import { sendText, sendCarousel, sendImage, sendButtons } from './messenger.js';
import { runAgent } from './ai.js';
import { loadHistory, saveHistory } from './session.js';
import { OPENER } from './knowledge.js';
import { buildCompareImage } from './compare-image.js';
...
// Мэндчилгээ мөн үү? (нээлтийг эхлүүлэх)
function isGreeting(t = '') {
  const s = t.toLowerCase().replace(/[!?.,]/g, '').trim();
  if (!s) return true;
  return /^(сайн уу|сайн байна уу|сайнуу|сайн байнуу|сайн|мэнд|hi|hello|hey|start|эхлэх|get started)$/.test(s);
}
...
export async function handleEvent(senderId, { text, payload }) {
  ...
  // НЭЭЛТ: анхны холбоо, Get Started, эсвэл мэндчилгээ → тогтмол 2 бөмбөлөг
  if (payload === 'GET_STARTED' || ((!history.length) && isGreeting(userText))) {
    for (const part of OPENER) await sendText(senderId, part);
    await saveHistory(senderId, [
      ...history,
      { role: 'user', content: userText },
      { role: 'assistant', content: OPENER.join('\n\n') },
    ]);
    return;
  }
  ...
  // 2 машин → харьцуулах зураг үүсгэж илгээнэ (алдвал carousel руу шилжинэ)
  const cars = result.vehicles || [];
  if (cars.length >= 2) {
    let sent = false;
    try {
      const png = await buildCompareImage(cars, compareTitle(cars));
      sent = await sendImage(senderId, png, 'compare.png');
    } catch (err) {
      console.error('compare image алдаа:', err.message);
    }
    if (!sent) await sendCarousel(senderId, cars.map(toElement));
  } else if (cars.length === 1) {
    await sendCarousel(senderId, cars.map(toElement));
  }

  const reply = stripMarkdown(result.text?.trim()) || 'Танд юугаар туслах вэ? 🚗';
  const q = result.searchQuery;
  if (cars.length >= 2 && q && SITE_URL) {
    // Тухайн маркийн БҮХ машиныг вэбсайтаас үзэх товч
    const url = `${SITE_URL}/vehicles?search=${encodeURIComponent(q)}`;
    await sendButtons(senderId, reply, [
      { type: 'web_url', url, title: 'Бүх сонголтыг үзэх' },
    ]);
  } else {
...
// Харьцуулах зургийн гарчиг (эхний машинаас)
function compareTitle(cars) {
  const v = cars[0] || {};
  const modelRoot = String(v.model || '').split(' ').slice(0, 2).join(' ');
  return `${v.brand || ''} ${modelRoot} — 2 сонголт харьцуулав`.trim();
}
