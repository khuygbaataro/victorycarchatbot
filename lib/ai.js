...
import { saveAppointment, saveUnknown } from './appointments.js';
...
const tools = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          color: {
            type: 'string',
            description: 'Гадна өнгө, ж: Цагаан, Хар, Улаан, Саарал, Сувдан цагаан',
          },
        },
      },
    },
  ...
  },
  {
    ...
    function: {
      name: 'log_unknown',
      description:
        'Хэрэглэгч БОТ МЭДЭХГҮЙ, өгөгдөлд байхгүй зүйл асуувал (тодорхой машины ' +
        'лизингэнд тэнцэх эсэх, засвар/түүх, байхгүй онцлог г.м.) тэр асуултыг ' +
        'бүртгэх. Дараа нь хэрэглэгчийг 8000-4020 руу залгахыг зөвлө.',
      parameters: {
        type: 'object',
        properties: {
          question: { type: 'string', description: 'Хэрэглэгчийн асуусан асуулт' },
        },
        required: ['question'],
      },
    },
  },
  {
    ...
    function: {
      ...
      description:
        'Хэрэглэгч биечлэн ирж үзэх цаг захиалахыг хүсвэл бүртгэх. Хэрэглэгч ирэх ' +
        'цагаа хэлмэгц дуудна. Энэ нь дилерийн admin-д харагдана.',
      parameters: {
        type: 'object',
        properties: {
          time: { type: 'string', description: 'Ирэх цаг/өдөр, ж: "Маргааш 14:00"' },
          name: { type: 'string', description: 'Ирж үзэх хүний нэр (асууж авсан)' },
          phone: { type: 'string', description: 'Холбогдох утасны дугаар (асууж авсан)' },
          carName: { type: 'string', description: 'Сонирхсон машин (мэдэгдэж байвал)' },
        },
        required: ['time'],
      },
    },
...
// history: [{role,content}]. senderId — цаг захиалгад. Буцаана: { text, vehicles }
export async function runAgent(history, userText, senderId) {
  ...
  let displayVehicles = [];
  let lastQuery = ''; // "бүх сонголт" холбоосонд ашиглах хайлтын үг
  ...
  for (let step = 0; step < 6; step++) {
      ...
      for (const tc of msg.tool_calls) {
        ...
        try {
          ...
          if (tc.function.name === 'search_vehicles') {
            if (args.query) lastQuery = String(args.query);
            ...
            const saved = await saveAppointment(senderId, args);
            result = { ok: true, ...saved };
          } else if (tc.function.name === 'log_unknown') {
            await saveUnknown(senderId, args.question);
            result = { ok: true };
          } else {
    ...
    return { text: msg.content || '', vehicles: displayVehicles, searchQuery: lastQuery };
  }

  return { text: 'Уучлаарай, дахин оролдоно уу.', vehicles: displayVehicles, searchQuery: lastQuery };
}
